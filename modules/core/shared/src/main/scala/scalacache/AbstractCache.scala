...
/**
  ...
  */
abstract class AbstractCache[F[_]: Async, V] extends Cache[F, V] with LoggingSupport {
  ...
  private def checkFlagsAndPut(key: String, value: V, ttl: Option[Duration])(implicit flags: Flags): F[Any] = {
  ...
  private def _caching(key: String, ttl: Option[Duration], f: => V)(
      implicit
      flags: Flags
  ): F[V] = _cachingF(key, ttl, Async[F].pure(f))
  ...
  private def _cachingF(key: String, ttl: Option[Duration], f: => F[V])(
    ...
    checkFlagsAndGet(key)
      .handleError { e =>
        if (logger.isWarnEnabled) logger.warn(s"Failed to read from cache. Key = $key", e)
        None
      }
      .flatMap {
        case Some(valueFromCache) =>
          Async[F].pure(valueFromCache)
        case None =>
